...
> This YAML is **illustrative of what the engine evaluates** — it is *generated by the PAP from a tenant's runtime-defined rules* (§8.7), **not hand-written or hardcoded**. Tenant admins create and edit policies on the fly; nothing is baked into the codebase.
...
- **Policy logic — user-defined at runtime, never hardcoded.** Tenant admins author roles, permissions, scopes, and **policy rules on the fly** via the PAP API; the **database is the source of truth** (rules stored as structured JSON, versioned). On publish, the PAP **compiles each tenant's rules into Cerbos policies and pushes them to the PDP** dynamically — effective in seconds (§3.4), no code change, no redeploy. *Optional* GitOps: a platform/tenant may *additionally* sync policies to Git for review/audit, and platform-wide *default* policies can ship as bundles — but **tenant policies are always runtime-editable and DB-backed**. Seed policies exist only as demo data.

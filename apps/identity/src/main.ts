...
async function bootstrap(): Promise<void> {
  ...
  // URI versioning: /v1/... (DESIGN §8.1). Health + JWKS are version-neutral.
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('Identity Service (IdP)')
    .setDescription('Multi-tenant access control — OIDC-style identity provider. DESIGN §5.')
    .setVersion('1.0.0')
    .addBearerAuth()
    .build();
  ...
  new Logger('Bootstrap').log(
    `identity listening on :${String(config.values.PORT)} (docs at /docs, jwks at /.well-known/jwks.json)`,
  );
}

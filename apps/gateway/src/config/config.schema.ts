 ...
 */
/**
 * The committed DEV default for the internal-token HMAC secret. It is published
 * in the repo, so a production gateway that signs the internal identity token
 * with it would let anyone who knows the default mint a valid signed token for
 * any sub/tid (confused-deputy / impersonation). Production MUST override it.
 */
export const DEV_INTERNAL_TOKEN_SECRET = 'dev-internal-token-secret-change-me';
...
export const configSchema = z
  .object({
     ...
     * RFC 8693). Defaults to a DEV value — production MUST override it (a boot
     * guard below refuses to start in production with the committed dev secret).
     */
    INTERNAL_TOKEN_SECRET: z.string().min(1).default(DEV_INTERNAL_TOKEN_SECRET),
  ...
  .superRefine((cfg, ctx) => {
    // Fail-closed production guard (mirrors the RLS superuser boot assertion):
    // a production gateway MUST NOT sign internal identity tokens with the
    // committed, repo-known dev secret — anyone who knows it could mint a valid
    // signed token for any sub/tid and impersonate any principal in any tenant.
    if (cfg.NODE_ENV === 'production' && cfg.INTERNAL_TOKEN_SECRET === DEV_INTERNAL_TOKEN_SECRET) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['INTERNAL_TOKEN_SECRET'],
        message:
          'refusing to boot in production with the committed dev secret ' +
          `('${DEV_INTERNAL_TOKEN_SECRET}'); set INTERNAL_TOKEN_SECRET to a unique secret`,
      });
    }
  });

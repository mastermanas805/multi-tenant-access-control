 ...
 *
 * The domain layer never imports HTTP; this filter is the single translation
 * point. NOTE: an UPSTREAM service's own 4xx/5xx body is NOT routed here — the
 * proxy controller streams it back verbatim so the §8.1 envelope minted by the
 * service is preserved end-to-end. This filter only handles errors raised INSIDE
 * the gateway (auth, routing, upstream-connectivity).
 */
@Catch()
export class GlobalExceptionFilter implements ExceptionFilter {
  ...
  private statusForDomainError(error: DomainError): number {
    ...
    if (error instanceof UpstreamUnavailableError) {
      // A timeout is 504; any other connectivity failure is 502.
      return error.reason === 'upstream_timeout'

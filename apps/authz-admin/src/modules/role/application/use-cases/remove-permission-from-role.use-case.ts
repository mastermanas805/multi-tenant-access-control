...
import { type RemovePermissionFromRoleCommand } from '../dto/role.commands';
import { type RoleView, toRoleView } from '../dto/role.view';

/**
 * Revokes a permission from a role. Honors optimistic concurrency (DESIGN §8.1):
 ...
 * aggregate records a RolePermissionRemovedEvent which the dispatcher publishes
 * after save (drives PIP cache invalidation within seconds — FR-8).
 */
@Injectable()
export class RemovePermissionFromRoleUseCase {
  ...
  public async execute(command: RemovePermissionFromRoleCommand): Promise<RoleView> {
    ...
    role.removePermission(command.permission, this.clock.now());
    await this.roles.save(role);

    // Publish RolePermissionRemovedEvent after persistence so the PIP cache is
    // told to invalidate only once the revocation is durable (DESIGN §3.4, FR-8).

...
export function buildDataSourceOptions(config: AppConfig): DataSourceOptions {
  return {
    ...
    migrationsTableName: 'authz_migrations',
  };
}
 ...
 * tenant isolation would be SILENTLY defeated. The unprivileged `authz_app` role
  ...
  if (isSuperuser || bypassesRls) {
    ...
    throw new Error(
      `RLS safety check failed: the runtime DB role ${reason}, which bypasses ` +
        ...
        'the API as the unprivileged `authz_app` role (set DB_USERNAME/DB_PASSWORD); ' +

 ...
 *
 * The aggregate has already bumped its version in memory (N -> N+1), so for an
 * existing row the DB must still be at `newVersion - 1`. We issue a single
 * compare-and-set:
 *
 ...
 *       via the normal `.save()` while still holding the lock in this transaction.
 ...
 *
 * Returns when the caller should proceed to persist the full aggregate (CAS won,
 * or it is a brand-new INSERT). Throws ConflictError on a stale write.
 */

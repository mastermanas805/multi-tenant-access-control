 ...
 *
 * Mapping rules:
 *   - DomainError subclasses -> their stable `code` + the status below. A
 *     ForbiddenError raised by the PEP carries `reason` + `decisionId`, so a PDP
 *     DENY renders as 403 with the deciding decision id (DESIGN §8.1/§8.2).

...
async function bootstrap(): Promise<void> {
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('Expense Service (PEP)')
    .setDescription(
      'Multi-tenant access control — worked Policy Enforcement Point example. DESIGN §4.3, §8.',
    )
    .setVersion('1.0.0')
    .addBearerAuth()
    .build();
  ...
  // Graceful shutdown: closes DB pool + Cerbos channel via lifecycle hooks.
  app.enableShutdownHooks();

  await app.listen(config.values.PORT);
  new Logger('Bootstrap').log(
    `expense listening on :${String(config.values.PORT)} (docs at /docs)`,
  );
}
